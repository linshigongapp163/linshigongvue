var util2 = {
	options: {
		ACTIVE_COLOR: "#007aff",
		NORMAL_COLOR: "#000",
		subpages: ["html/tab-webview-subpage-contact.html","html/tab-webview-subpage-contact.html"]
	},
	/**
	 *  简单封装了绘制原生view控件的方法
	...
	initSubpage: function(aniShow) {
		var subpage_style = {
			...
			subpages = util2.options.subpages,
	...
	toggleNview: function(currIndex) {
		...
		util2.updateSubNView(currIndex, util2.options.ACTIVE_COLOR);
		util2.updateSubNView(currIndex + 1, util2.options.ACTIVE_COLOR);
		// 重绘兄弟tag 反之排除当前点击的icon和text
		for(var i = 0; i < 4; i++) {
			if(i !== currIndex && i !== currIndex + 1) {
				util2.updateSubNView(i, util2.options.NORMAL_COLOR);
			}
		}
	},
	/*
...
//			nviewObj = self.getStyle().subNViews[0], // 获取nview对象的属性
			nviewObj = h.getStyle().subNViews[0],
			currTag = nviewObj.tags[currIndex]; // 获取当前需重绘的tag

		nviewEvent.drawText(currTag.text, currTag.position, util2.changeColor(currTag.textStyles, color), currTag.id);
	}
};
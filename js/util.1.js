var util = {
	options: {
		...
		ACTIVE_COLOR: "#FF0000",
		NORMAL_COLOR: "#ccc",		
		subpages: ["html/tab-webview-subpage-contact.html","html/mycollection.html","html/push.html","html/tab-webview-subpage-contact.html"]
	},
	/**
	 *  简单封装了绘制原生view控件的方法
	...
	initSubpage: function(aniShow) {
		...
		// 初始化第一个tab项为首次显示
		temp[self.id] = "true";
		console.log('util.js---L41; temp=='+JSON.stringify(temp))
		mui.extend(aniShow, temp);
		console.log('util.js---L43;aniShow=='+JSON.stringify(aniShow))
		// 初始化绘制首个tab按钮
		util.toggleNview(0);

		for(var i = 0, len = subpages.length; i < len; i++) {
console.log('util.js--L48;i=='+i)
			if(!plus.webview.getWebviewById(subpages[i])) {
				var sub = plus.webview.create(subpages[i], subpages[i], subpage_style);
				console.log('util.js---L51; sub==='+JSON.stringify(sub))
				//初始化隐藏
				// sub.hide();
				// append到当前父webview
				self.append(sub);
	...
	changeSubpage: function(targetPage, activePage, aniShow) {
		console.log('util.js---L64;targetPage=='+targetPage+';activePage=='+activePage+';anishow=='+aniShow);
		//若为iOS平台或非首次显示，则直接显示
		if(mui.os.ios || aniShow[targetPage]) {
			console.log('util.js---iosORaniShow')
			plus.webview.show(targetPage);
		} else {
			//否则，使用fade-in动画，且保存变量
			console.log('util.js--L71')
			var temp = {};
			temp[targetPage] = "true";
			mui.extend(aniShow, temp);
			plus.webview.show(targetPage, "fade-in", 300);
		}
		console.log('util.js---L77')
		//隐藏当前 除了第一个父窗口
		if(activePage !== plus.webview.getLaunchWebview()) {
			console.log('util.js--L80;activePage==='+JSON.stringify(activePage))
